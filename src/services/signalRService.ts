...
import { MessageModel, UserSummaryModel } from 'src/models/chat/chatModel';
...
export default class SignalRService {
  ...
  sendMessage(sender: UserSummaryModel, recipient: UserSummaryModel, message: string) {
  ...
  getMessages(pageNumber = 1, pageSize = 100): Promise<Array<MessageModel>> {
    return (
      axios.get(`${config.chatApiUrl}/users/me/conversations/${this.conversationId}/messages?pageNumber=${pageNumber}&pageSize=${pageSize}`)
