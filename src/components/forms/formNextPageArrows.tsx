...
export default class FormNextPageArrows extends React.Component<{
  mainClick?: () => void;
  secondaryClickOptions?: clickOptionsModel;
}> {
  state = {
  ...
  render(): JSX.Element {
    return this.props.secondaryClickOptions ? (
      <>
        <FadeInAnimator
          visible={this.state.showNextStepsOptions}
          style={{
            position: 'absolute',
            bottom: 70,
            right: 20,
          ...
          {this.props.secondaryClickOptions.map((clickOption: { label: string; onClick: () => void }) => (
            <TouchableOpacity
              key={clickOption.label}
