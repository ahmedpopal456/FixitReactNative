...
const FixRequestActions: FunctionComponent<NavigationProps<FixProps>> = (
  props: NavigationProps<FixProps>,
): JSX.Element => {
  const navigation = useNavigation();

  const { title, id, notificationId } = props.route.params;
  ...
  const [acceptCraftsmanModalOpen] = useState<boolean>(false);
  ...
  const handleCraftsmanMakeOffer = (): void => {
    if (fix) {
  ...
  const handleCraftsmanReject = (): void => {
    setRejectFixModalOpen(true);
  };

  const handleClientRefuse = (): void => {
    notificationService.deleteNotificationsByIds(user.userId as string, [notificationId]);
    navigation.navigate('HomeScreen');
  };

  const handleClientAccept = (): void => {
  ...
  const BottomButtons = () => {
    if (id === 'fix_client_request') {
      return generatePairOfButton(
        { onClick: handleCraftsmanMakeOffer, text: 'Make an offer' },
        { onClick: handleCraftsmanReject, text: 'Reject' },
      );
    }
    if (id === 'fix_craftsman_response') {
      return generatePairOfButton(
        { onClick: handleClientAccept, text: 'Accept' },
        { onClick: handleClientRefuse, text: 'Reject' },
      );
    }
          ...
          secondaryClickOptions={[
            {
              ...
              onClick: handleClientSubmitFixRequestConfirm,
            },
          ]}
        />
      );
    ...
    <>
      ...
      <FadeInAnimator
        ...
        <View
          ...
          <Button onPress={onCloseModal} style={{ marginTop: 20 }}>
            Confirm
          </Button>
        </View>
      </FadeInAnimator>
      <FadeInAnimator
        ...
        <View
          ...
          <View
            ...
            <Button onPress={handleClientRefuse} width={150} color="accent">
              Refuse
            </Button>
          </View>
        </View>
...
export default FixRequestActions;
