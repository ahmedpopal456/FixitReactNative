...
export default class B2CClient {
  ...
  public async signUp(b2cSignInUpParams: B2CSignInUpParams): Promise<MSALResult> {
    ...
    return this.publicClientApplication.acquireToken({ ...rest, authority });
  }
  ...
  public async signIn(b2cSignInUpParams: B2CSignInUpParams): Promise<MSALResult> {
    ...
    try {
      ...
      if (error.message.includes(B2CClient.B2C_PASSWORD_CHANGE)
  ...
  : Promise<MSALResult> {
    ...
    if (account) {
      return this.publicClientApplication.acquireTokenSilent(
        { ...b2cAcquireTokenSilentParams, account },
      );
    }
